...
VERSION='noto-correlated-facility-v1';RHOS=[0,.1,.25]
...
def main(out,workers):
 d=json.loads((out/'run_design.json').read_text());a=args_from(d,out);target=out/'correlated_facility_v1';(target/'tables').mkdir(parents=True,exist_ok=True);cache=CheckpointStore(target/'checkpoints');rows=[];levels=[[0,.25,.5,1],[0,.5,1],[0,.5,1],[0,.5,1],[0,.25,.5,1]]
 for rho in RHOS:
