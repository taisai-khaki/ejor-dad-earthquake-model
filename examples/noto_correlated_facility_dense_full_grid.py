...
VERSION='noto-correlated-facility-full-v1';RHOS=[0,.025,.05,.075,.1,.125,.15,.2,.25]
...
def main(out,workers):
 d=json.loads((out/'run_design.json').read_text());a=args_from(d,out);target=out/'correlated_facility_full_v1';(target/'tables').mkdir(parents=True,exist_ok=True);cache=CheckpointStore(target/'checkpoints');rows=[];levels=[[0,.25,.5,.75,1]]*5
 for rho in RHOS:
  ...
  for index,y in candidates:
   ...
   if cache.exists(key):
    ...
    reused=None
    reduced_root=out/'correlated_facility_v1'/'checkpoints'
    for old_path in reduced_root.glob(f'*rho{rho:.2f}_*.json'):
     old=json.loads(old_path.read_text())
     if np.allclose(old.get('y',[]),y):reused=old;break
    if reused is not None:
     cache.save(key,reused);results.append(reused)
