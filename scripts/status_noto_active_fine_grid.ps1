...
if (-not (Test-Path -LiteralPath $StatusPath)) {
    Write-Output "No Noto active fine-grid status file found at $StatusPath"
    exit 0
}
...
$TablePath = Join-Path $OutputDir "tables\table_noto_active_fine_grid_summary.csv"
if (Test-Path -LiteralPath $TablePath) {
    Write-Output ""
    Import-Csv -LiteralPath $TablePath |
        Select-Object rho,best_objective,best_y_json,objective_using_rho0_policy,delta_rho_value,policy_changed_from_rho0,second_best_gap |
