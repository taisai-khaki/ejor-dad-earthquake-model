...
try {
    Write-RunnerStatus -Status "running" -Block "grid" -Message "Graded-response full Noto grid sweep started."
    $gridArgs = @(
    ...
    Write-RunnerStatus -Status "completed" -Block "complete" -Message "Graded-response grid, certificate, and diagnostics completed." -ExitCode 0
}
catch {
