...
$ProgressLogPath = Join-Path $LogDir ("noto_moment_progress_" + $Timestamp + ".log")
$ProcessLogPath = Join-Path $LogDir ("noto_moment_process_" + $Timestamp + ".log")
$env:PYTHONPATH = "$Repo\src;$Repo\examples"
$env:EJOR_LOG_PATH = $ProgressLogPath

$Arguments = @(
    "$Repo\examples\noto_moment_ambiguity_experiment.py",
