...
try {
    Write-RunnerStatus -Status "running" -Message "Restart-safe density-cap and graded-response sensitivity enumeration started."
    Set-Location $Repo
    $arguments = @(
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Joint sensitivity runner failed with exit code $LASTEXITCODE."
    }
    Write-RunnerStatus -Status "completed" -Message "Restart-safe density-cap and graded-response sensitivity enumeration completed." -ExitCode 0
}
catch {
