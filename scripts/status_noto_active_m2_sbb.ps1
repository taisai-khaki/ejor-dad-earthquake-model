...
if (-not (Test-Path -LiteralPath $StatusPath)) {
    Write-Output "No active critical-link SBB status file found at $StatusPath"
    exit 0
}
...
$TablePath = Join-Path $OutputDir "tables\table_noto_active_critical_m2_sbb.csv"
if (Test-Path -LiteralPath $TablePath) {
    Write-Output ""
    Import-Csv -LiteralPath $TablePath |
        Select-Object rho,objective,lower_bound,absolute_gap,relative_gap_percent,selected_y_json,delta_rho_value,nodes_processed,converged,termination_reason |
